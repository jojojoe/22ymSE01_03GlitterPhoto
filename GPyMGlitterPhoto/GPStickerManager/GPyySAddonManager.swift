...
class GPpYymAddonManager: NSObject {
    
    static var `default` = GPpYymAddonManager()
    
    // sticker
    var addonStickersList: [GPyPmStickerTouchView] = []
    var currentStickerAddon: GPyPmStickerTouchView? = nil
    ...
    */
    // text
    var addonTextsList: [GPyymTextTouchView] = []
    var currentTextAddon: GPyymTextTouchView? = nil
    let textFontSize =  30
    ...
    var shapeAddonReplaceBarStatusBlock:((_ shapeAddon: GPyPmStickerTouchView)->Void)?
    var textAddonReplaceBarStatusBlock:((_ textAddon: GPyymTextTouchView)->Void)?
    var removeStickerAddonActionBlock:(()->Void)?
    var textAddonSingleClickStatusBlock:((_ textAddon: GPyymTextTouchView)->Void)?
...
// sticker
extension GPpYymAddonManager {
    
    func addNewStickerAddonWithStickerImage(stickerImage: UIImage, stickerItem: GPCyStickerItem, atView stickerCanvasView:UIView) {
        
        cancelCurrentAddonHilightStatus()
        
        let stickerView: GPyPmStickerTouchView = GPyPmStickerTouchView.init(withImage:stickerImage , viewSize: CGSize.init(width: stickerWidth, height: stickerWidth))
    ...
    func removeStickerTouchView(stickerTouchView: GPyPmStickerTouchView) {
    ...
    func selectedOrAddNewCurrentTextAddon(canvasView: UIView, isAddNew: Bool) -> Bool {
        if isAddNew == true {
            ...
            if let textAddon = currentTextAddon {
                ...
                textAddon.replaceCanvasContentAlpha(alpha: 0)
...
//        }
        ...
        let textView: GPyymTextTouchView = GPyymTextTouchView.init(withAttributeString: attri, canvasBounds: canvasView.bounds)
        ...
        textView.deleteActionBlock = { [weak self] contentTouchView in
            guard let `self` = self else { return }
            self.removeTextTouchView(textTouchView: contentTouchView as! GPyymTextTouchView)
    ...
    func removeTextTouchView(textTouchView: GPyymTextTouchView) {
...
extension GPpYymAddonManager: TouchStuffViewDelegate {
...
//        cancelCurrentAddonHilightStatus()
        ...
        if className.contains("GPyymTextTouchView") {
            let textAddon: GPyymTextTouchView = sender as! GPyymTextTouchView
...
//        cancelCurrentAddonHilightStatus()
        ...
        if className.contains("GPyPmStickerTouchView") {
            let stickerAddon: GPyPmStickerTouchView = sender as! GPyPmStickerTouchView
        ...
        } else if className.contains("GPyymTextTouchView") {
            let textAddon: GPyymTextTouchView = sender as! GPyymTextTouchView
            currentTextAddon = textAddon
            textAddonReplaceBarStatusBlock?(textAddon)
            textAddonSingleClickStatusBlock?(textAddon)
